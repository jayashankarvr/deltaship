...
/// Known platform values that match the Platform enum in vbdp-core.
/// Both hyphen and underscore variants are included for flexible matching.
///
/// # P3 Issue #105 Fix: Platform List Centralized
///
/// This now references the centralized platform list from vbdp-core::Platform
/// instead of maintaining a separate hardcoded list.
pub const KNOWN_PLATFORMS: &[&str] = vbdp_core::Platform::all_platform_variants();
...
/// This now references the centralized platform list from vbdp-core::Platform
/// instead of maintaining a separate hardcoded list.
pub const CANONICAL_PLATFORMS: &[&str] = vbdp_core::Platform::all_platforms();
...
///
/// **Note:** The blake3 crate is not a dependency of vbdp-db (to keep the database
/// layer lightweight). The actual Blake3 hashing is done in the vbdp-crypto crate.
/// If you need to verify this constant matches blake3::OUT_LEN, add this assertion
/// to vbdp-crypto where blake3 is already a dependency:
///
/// ```ignore
