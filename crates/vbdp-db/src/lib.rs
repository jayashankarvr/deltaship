...
//! VBDP Publisher Toolkit and Client Patcher.
//!
//! # Overview
//!
//! VBDP uses SQLite for local storage in both the publisher and client components:
//!
...
//! ```no_run
//! use std::path::Path;
//! use vbdp_db::{PublisherDb, NewBinary, NewVersion};
//!
//! # async fn example() -> vbdp_db::Result<()> {
//! // Open or create a publisher database
//! let db = PublisherDb::open(Path::new(".vbdp/publisher.db")).await?;
//! db.init().await?;
//!
//! // Register a binary
...
//! ```no_run
//! use std::path::Path;
//! use vbdp_db::{ClientDb, NewManagedBinary, NewUpdateRecord};
//!
//! # async fn example() -> vbdp_db::Result<()> {
//! // Open or create a client database
//! let db = ClientDb::open(Path::new("/var/lib/vbdp/client.db")).await?;
//! db.init().await?;
//!
