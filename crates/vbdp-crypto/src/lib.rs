//! Cryptographic primitives for VBDP (Verified Binary Distribution Protocol)
//!
//! This crate provides:
...
//! // vbdp_crypto::save_signing_key(&key, &path, Some(&passphrase), SecurityLevel::Standard);
...
mod integration_tests {
    ...
    fn temp_file(name: &str) -> PathBuf {
        ...
        temp_dir.join(format!("vbdp_integration_{}", name))
    }
    ...
    fn test_sign_verify_workflow() {
        ...
        assert!(
            content.contains("-----BEGIN VBDP ENCRYPTED SIGNING KEY-----"),
        ...
        assert!(
            pub_content.contains("-----BEGIN VBDP VERIFYING KEY-----"),
            "Verifying key should have correct header"
        );
        assert!(
            pub_content.contains("-----END VBDP VERIFYING KEY-----"),
    ...
    fn test_sign_verify_workflow_unencrypted() {
        ...
        let content = fs::read_to_string(&signing_key_path).unwrap();
        assert!(
            content.contains("-----BEGIN VBDP SIGNING KEY-----"),
