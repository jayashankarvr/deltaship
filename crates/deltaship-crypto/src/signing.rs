...
/// Domain-separation tag for the canonical Deltaship signing payload (17 bytes:
/// the ASCII string `DELTASHIP-sig-v1` followed by a single `0x00` byte).
pub const SIGNING_DOMAIN_TAG: &[u8] = b"DELTASHIP-sig-v1\x00";

/// Build the canonical Ed25519 signing payload ("DELTASHIP-sig-v1").
///
...
///
///   1. The 17-byte domain tag [`SIGNING_DOMAIN_TAG`].
///   2. The raw 32 bytes of the BLAKE3 hash of the binary (NOT hex).
///   3. The UTF-8 bytes of the version string.
///
/// The domain tag is fixed at 17 bytes and the BLAKE3 hash is fixed at 32 bytes,
...
mod tests {
    ...
    fn test_signing_payload_golden_vector() {
        ...
        expected.extend_from_slice(b"DELTASHIP-sig-v1\x00");
