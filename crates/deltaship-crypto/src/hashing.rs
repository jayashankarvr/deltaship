//! Blake3 hashing utilities for Deltaship.
//!
...
// This ensures consistency with BLAKE3_HASH_SIZE constant in deltaship-db
const _: () = assert!(
    blake3::OUT_LEN == 32,
...
impl Hash {
    ...
    ///
    /// ```
    /// use deltaship_crypto::{hash_bytes, Hash};
    ///
...
mod tests {
    ...
    fn test_hash_file() {
        ...
        let temp_file = temp_dir.join("deltaship_test_hash_file.bin");
    ...
    fn test_hash_large_file() {
        let temp_dir = std::env::temp_dir();
        let temp_file = temp_dir.join("deltaship_test_large_file.bin");
    ...
    fn test_hash_very_large_file() {
        let temp_dir = std::env::temp_dir();
        let temp_file = temp_dir.join("deltaship_test_very_large_file.bin");
    ...
    fn test_hash_boundary_sizes() {
        ...
        for size in sizes {
            ...
            let temp_dir = std::env::temp_dir();
            let temp_file = temp_dir.join(format!("deltaship_test_boundary_{}.bin", size));
            std::fs::write(&temp_file, &data).unwrap();
            let file_hash = hash_file(&temp_file).unwrap();
    ...
    fn test_hash_file_with_special_content() {
        ...
        for (name, content) in test_cases {
            let temp_file = temp_dir.join(format!("deltaship_test_special_{}.bin", name));
    ...
    fn test_hash_concurrent_access() {
        ...
        let temp_dir = std::env::temp_dir();
        let temp_file = temp_dir.join("deltaship_test_concurrent.bin");
    ...
    fn test_sha256_file_basic() {
        let temp_dir = std::env::temp_dir();
        let temp_file = temp_dir.join("deltaship_test_sha256.bin");
    ...
    fn test_sha256_file_with_progress() {
        let temp_dir = std::env::temp_dir();
        let temp_file = temp_dir.join("deltaship_test_sha256_progress.bin");
    ...
    fn test_sha256_and_blake3_consistency() {
        ...
        let temp_dir = std::env::temp_dir();
        let temp_file = temp_dir.join("deltaship_test_dual_hash.bin");
