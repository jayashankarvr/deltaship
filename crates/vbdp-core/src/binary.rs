...
impl FromStr for BinaryId {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        Uuid::parse_str(s)
            .map(Self)
            .map_err(|e| VbdpError::InvalidBinaryId(e.to_string()))
    }
}
...
impl BinaryName {
    pub fn new(name: &str) -> Result<Self, VbdpError> {
        if name.is_empty() {
            return Err(VbdpError::InvalidBinaryName(
...
impl Platform {
    ...
    ///
    /// This is the single source of truth for supported platforms in the VBDP system.
...
impl FromStr for Platform {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            ...
            _ => Err(VbdpError::InvalidPlatform(s.to_string())),
        }
    }
}
