//! Core error types for VBDP.
//!
//! # Error Handling Conventions
//!
//! This library crate uses custom error types with an implicit result pattern:
//! - `VbdpError`: Custom error type for core domain operations
//! - No type alias defined - consumers use `Result<T, VbdpError>` directly
//!
...
/// Error types for VBDP operations
#[derive(Debug, Error)]
pub enum VbdpError {
...
impl From<serde_json::Error> for VbdpError {
    fn from(err: serde_json::Error) -> Self {
        VbdpError::SerializationError(err.to_string())
    }
}
