//! Semantic versioning support for Deltaship.
//!
...
//!
//! **Why this matters for Deltaship**: When the server compares client versions for updates,
...
impl Version {
    ...
    pub fn with_prerelease(
        ...
        patch: u64,
        prerelease: &str,
    ) -> Result<Self, DeltashipError> {
        let pre = semver::Prerelease::new(prerelease)
            .map_err(|e| DeltashipError::InvalidVersion(e.to_string()))?;
