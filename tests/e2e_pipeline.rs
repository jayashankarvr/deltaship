...
//!
//! Real integration test that exercises the full VBDP pipeline:
//!
//!   1. Spawn `vbdp-server` on a random free TCP port.
//!   2. Initialize a publisher workspace + signing key.
//!   3. Build a tiny `myapp` v1 binary, register/sign/publish it.
//!   4. Spawn `vbdp-updater` -- assert exit code 2 (first install)
//!      and verify the installed binary's BLAKE3 matches the published checksum.
//!   5. Build `myapp` v2, register/sign/publish.
//!   6. Spawn `vbdp-updater` again -- assert exit code 2 (updated)
//!      and verify the install path now contains v2.
//!
//! Run with: `cargo test --test e2e_pipeline -p vbdp-integration-tests -- --ignored`
//!
...
//! `vbdp-server`, `vbdp-publisher`, and `vbdp-updater` binaries exist under
...
impl ServerGuard {
    ...
    fn start(server_bin: &Path, data_dir: &Path, port: u16, timeout: Duration) -> Self {
        ...
        cmd.arg("--host")
            ...
            .stdout(Stdio::null())
            .stderr(Stdio::piped());
        let child = cmd.spawn().expect("spawn vbdp-server");
                ...
                Err(_) => {
                    ...
                    if let Some(child) = guard.child.as_mut() {
                        if let Ok(Some(status)) = child.try_wait() {
                            ...
                            panic!(
                                "vbdp-server exited early with {:?}; stderr:\n{}",
                                status, buf
                            );
                        }
                    }
...
fn e2e_pipeline_full() {
    ...
    let server_bin = locate_binary("vbdp-server");
    let publisher_bin = locate_binary("vbdp-publisher");
    let updater_bin = locate_binary("vbdp-updater");
    ...
    let pubkey_src = publisher_dir.join(".vbdp/keys/public.key");
