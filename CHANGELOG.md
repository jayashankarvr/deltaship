...
> user that hosts the canonical Deltaship repository.
...
Deltaship follows [Semantic Versioning 2.0.0](https://semver.org/spec/v2.0.0.html)
...
- Renames, removes, or repurposes a CLI flag or subcommand on
  `deltaship-publisher`, `deltaship-server`, `deltaship-client`, or `deltaship-updater`.
...
- **zstd-compressed binary deltas.** The diff pipeline produces
  ...
  Compression is applied transparently in `deltaship-diff` and unwrapped on
  the client during patch application.
- **`deltaship-updater` sidecar binary.** A small companion binary
  (`crates/deltaship-client/src/bin/deltaship-updater.rs`) intended to be spawned
...
- **End-to-end integration test (`tests/end_to_end.rs`).** Spawns a
  real `deltaship-server`, drives `deltaship-publisher` to publish two versions
...
- **Docker Compose deployment for the update server
  ...
  a starting point for running `deltaship-server` with a persistent volume
...
- **Multi-platform CI publish template
  ...
  feeds into a fan-in `deltaship-publisher register/sign/publish` step
  using GitHub Secrets. README documents required secrets and known
  gaps in the publisher CLI for non-interactive use.
- **End-to-end pipeline test exercising `deltaship-updater`
  (`tests/e2e_pipeline.rs`).** Spawns the server, runs the publisher
  for v1+v2, runs `deltaship-updater` as a child process, and asserts on
...
- **Publisher upload checksum is now BLAKE3** to match what the server
  computes and validates (`crates/deltaship-publisher/src/commands/publish.rs`).
...
- **Diff upload multipart form now includes the `platform` field**
  (`crates/deltaship-publisher/src/api_client.rs`). The server has always
...
- **Public verifying-key files are written with mode `0644`** instead
  ...
  (`crates/deltaship-crypto/src/keys.rs`). Signing keys remain `0600`.
...
- **CLIENT-P0-1: Path validation on rollback.** The rollback command
  ...
  (`crates/deltaship-client/src/commands/rollback.rs`).
- **CLIENT-P0-2: Signature verification in `download_update_only`.**
  ...
  (`crates/deltaship-client/src/commands/update.rs`).
- **CRYPTO-P1-1: Decoded signing-key bytes are now wrapped in
  ...
  (`crates/deltaship-crypto/src/keys.rs`).
...
- **DB-P1-1: `diff_algorithm` values are now validated** against the
  ...
  shared helper (`crates/deltaship-db/src/publisher.rs`).
...
- **CLIENT-P1-1: Backup signature verification on rollback.** The
  ...
  (`crates/deltaship-client/src/patcher.rs`,
  `crates/deltaship-client/src/commands/rollback.rs`).
- **PROJECT-P1-1 / P1-2: Empty security-test stubs replaced with real
  ...
  (`crates/deltaship-client/tests/security_tests.rs`,
  `crates/deltaship-client/tests/version_validation_tests.rs`).
- **PROJECT-P1-3 / P1-4: Placeholder contact addresses standardized**
  on `<role>@deltaship.example.com` (RFC 2606 reserved domain) across the
...
[Unreleased]: https://github.com/REPLACE_ME_ORG/deltaship/compare/v0.2.0...HEAD
[0.2.0]: https://github.com/REPLACE_ME_ORG/deltaship/releases/tag/v0.2.0
[0.1.0]: https://github.com/REPLACE_ME_ORG/deltaship/releases/tag/v0.1.0
