...
Thank you for your interest in contributing to Deltaship!
...
3. Include:
   - Deltaship version (run `deltaship-client version`, `deltaship-publisher version`, or `deltaship-server --version`)
...
```bash
# Publisher toolkit
cargo build -p deltaship-publisher

# Update server
cargo build -p deltaship-server

# Client patcher
cargo build -p deltaship-client

# Core library
cargo build -p deltaship-core
```
...
```bash
cargo test -p deltaship-core
cargo test -p deltaship-crypto
cargo test -p deltaship-diff
```
