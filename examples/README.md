...
This directory contains example scripts demonstrating the VBDP (Verified Binary Distribution Protocol) workflow.
...
- Create a temporary demo environment in `/tmp/vbdp-demo`
- Run all steps of the VBDP workflow
...
| 3 | **Publisher Init** - Initializes VBDP project, generates Ed25519 keypair |
...
Creating directory structure at /tmp/vbdp-demo

=== Step 3: Initializing VBDP publisher project ===
Initializing VBDP project...
  Created .vbdp/
  Created .vbdp/keys/
...
```bash
DEMO_DIR="/tmp/vbdp-demo"    # Where to create demo files
...
### Publisher (`vbdp-publisher`)

The publisher toolkit manages binary releases:

```bash
vbdp-publisher init                    # Initialize project
vbdp-publisher keygen                  # Generate signing keypair
vbdp-publisher register --name APP --version X.Y.Z --file PATH --platform PLATFORM
vbdp-publisher sign --name APP --version X.Y.Z
vbdp-publisher publish --name APP --version X.Y.Z --server-url URL
vbdp-publisher list                    # List registered versions
```
...
```bash
vbdp-server --host 0.0.0.0 --port 8080 --data-dir ./data
```
...
```bash
vbdp-client add --name APP --path PATH --public-key-file KEY
vbdp-client list                       # List managed binaries
vbdp-client status                     # Show update status
vbdp-client --check-now                # One-time update check
vbdp-client --daemon                   # Run as background daemon
```
