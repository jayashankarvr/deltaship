#!/bin/bash
# Deltaship End-to-End Demo
# This script demonstrates the complete publisher -> server -> client workflow.
#
# The Verified Binary Distribution Protocol (Deltaship) enables secure, signed
...
DELTASHIP_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PUBLISHER_BIN="${DELTASHIP_ROOT}/target/release/deltaship-publisher"
SERVER_BIN="${DELTASHIP_ROOT}/target/release/deltaship-server"
CLIENT_BIN="${DELTASHIP_ROOT}/target/release/deltaship-client"

# Use debug binaries as fallback
if [ ! -f "$PUBLISHER_BIN" ]; then
    PUBLISHER_BIN="${DELTASHIP_ROOT}/target/debug/deltaship-publisher"
fi
if [ ! -f "$SERVER_BIN" ]; then
    SERVER_BIN="${DELTASHIP_ROOT}/target/debug/deltaship-server"
fi
if [ ! -f "$CLIENT_BIN" ]; then
    CLIENT_BIN="${DELTASHIP_ROOT}/target/debug/deltaship-client"
fi
...
check_binary "deltaship-publisher" "$PUBLISHER_BIN" || exit 1
check_binary "deltaship-server" "$SERVER_BIN" || exit 1
check_binary "deltaship-client" "$CLIENT_BIN" || exit 1
...
#------------------------------------------------------------------------------
# Step 3: Initialize Deltaship publisher project
#------------------------------------------------------------------------------
echo_step "Step 3: Initializing Deltaship publisher project"

cd "$PUBLISHER_DIR"

echo_info "The publisher init command creates:"
echo "  - .deltaship/ directory structure"
...
echo_cmd "deltaship-publisher init"
echo "" | "$PUBLISHER_BIN" init 2>&1 || {
    # If init prompts for passphrase, try piping empty strings
    echo -e "\n" | "$PUBLISHER_BIN" init 2>&1 || true
}

echo_info "Project initialized. Keys generated in .deltaship/keys/"
ls -la "$PUBLISHER_DIR/.deltaship/keys/" 2>/dev/null || echo_info "(keys directory)"

# Copy public key for client use later
PUBLIC_KEY_FILE="$PUBLISHER_DIR/.deltaship/keys/public.key"
...
echo_cmd "deltaship-publisher register --name $APP_NAME --version 1.0.0 --file $DEMO_DIR/app-v1.0.0 --platform $PLATFORM"
...
echo_info "When registering a new version, Deltaship automatically generates"
echo_info "binary diffs from previous versions for efficient delta updates."

echo_cmd "deltaship-publisher register --name $APP_NAME --version 1.1.0 --file $DEMO_DIR/app-v1.1.0 --platform $PLATFORM"
"$PUBLISHER_BIN" register \
    --name "$APP_NAME" \
    --version "1.1.0" \
...
echo_cmd "deltaship-publisher list"
"$PUBLISHER_BIN" list
...
echo_cmd "deltaship-publisher sign --name $APP_NAME --version 1.0.0"
echo "" | "$PUBLISHER_BIN" sign --name "$APP_NAME" --version "1.0.0" 2>&1 || {
    echo -e "\n" | "$PUBLISHER_BIN" sign --name "$APP_NAME" --version "1.0.0" 2>&1 || true
}

# Sign v1.1.0
echo_cmd "deltaship-publisher sign --name $APP_NAME --version 1.1.0"
...
#------------------------------------------------------------------------------
echo_step "Step 8: Starting Deltaship update server"
...
echo_cmd "deltaship-server --host $SERVER_HOST --port $SERVER_PORT --data-dir $DEMO_DIR/server-data &"
"$SERVER_BIN" \
    --host "$SERVER_HOST" \
    --port "$SERVER_PORT" \
...
echo_cmd "deltaship-publisher publish --name $APP_NAME --version 1.0.0 --server-url $SERVER_URL"
...
echo ""
echo_cmd "deltaship-publisher publish --name $APP_NAME --version 1.1.0 --server-url $SERVER_URL"
"$PUBLISHER_BIN" publish \
    --name "$APP_NAME" \
    --version "1.1.0" \
    --server-url "$SERVER_URL"
...
# Deltaship Client Configuration
...
if [ -f "$CLIENT_DIR/publisher.pub" ]; then
    echo_cmd "deltaship-client --config $CLIENT_CONFIG add --name $APP_NAME --path $INSTALLED_BINARY --public-key-file $CLIENT_DIR/publisher.pub"
    "$CLIENT_BIN" \
...
echo_cmd "deltaship-client --config $CLIENT_CONFIG list"
...
echo_cmd "deltaship-client --config $CLIENT_CONFIG --check-now"
...
echo_cmd "deltaship-client --config $CLIENT_CONFIG status"
...
echo ""
echo -e "${GREEN}Summary of the Deltaship workflow:${NC}"
echo ""
echo "  1. PUBLISHER SIDE:"
echo "     - Initialize project with 'deltaship-publisher init'"
echo "     - Register versions with 'deltaship-publisher register'"
echo "     - Sign versions with 'deltaship-publisher sign'"
echo "     - Publish to server with 'deltaship-publisher publish'"
echo ""
echo "  2. SERVER SIDE:"
echo "     - Run 'deltaship-server' to host update infrastructure"
echo "     - Serves binaries, diffs, and signatures via REST API"
echo ""
echo "  3. CLIENT SIDE:"
echo "     - Add binaries to manage with 'deltaship-client add'"
echo "     - Check for updates with 'deltaship-client --check-now'"
echo "     - Run as daemon with 'deltaship-client --daemon'"
echo ""
echo -e "${YELLOW}Key files created during this demo:${NC}"
echo "  - Publisher keys: $PUBLISHER_DIR/.deltaship/keys/"
echo "  - Publisher DB:   $PUBLISHER_DIR/.deltaship/publisher.db"
