...
**Document:** Operational procedures for running VBDP infrastructure
...
This guide provides operational procedures, maintenance tasks, troubleshooting steps, and best practices for running VBDP (Version-Aware Binary Differential Update System) in production.
...
psql -h database-host -U vbdp_server -d vbdp -c "SELECT COUNT(*) FROM apps;"
...
aws s3 ls s3://vbdp-updates-prod/ | head
...
journalctl -u vbdp-server --since "1 hour ago" -p err

# Or file logs
grep ERROR /var/log/vbdp/server.log | tail -n 20
```
...
# Manual (if needed)
psql -h database-host -U vbdp_server -d vbdp -c "VACUUM ANALYZE;"
```
...
psql -h database-host -U vbdp_server -d vbdp
...
```
# Check database size
psql -h database-host -U vbdp_server -d vbdp -c "\l+"

# Check table sizes
psql -h database-host -U vbdp_server -d vbdp -c "
SELECT
  tablename,
...
```
# List old diffs
aws s3 ls s3://vbdp-updates-prod/diffs/ --recursive

# Delete diffs older than 6 months
aws s3 ls s3://vbdp-updates-prod/diffs/ --recursive \
  | awk '{if ($1 < "2025-07-01") print $4}' \
  | xargs -I {} aws s3 rm s3://vbdp-updates-prod/{}
```
...
/var/log/vbdp/*.log {
    ...
    missingok
    notifempty
    create 0640 vbdp vbdp
    sharedscripts
    postrotate
        systemctl reload vbdp-server > /dev/null 2>&1 || true
    endscript
}
```
...
Test:
```
logrotate -d /etc/logrotate.d/vbdp
```
...
psql -c "SELECT * FROM pg_stat_activity WHERE usename = 'vbdp_server';"
...
sha256sum /etc/vbdp-server/publisher-keys/*.pub
```

**4. Update Dependencies:**
```
# Check for security updates
apt list --upgradable | grep vbdp

# Or for containers
docker pull vbdp/server:latest
docker images --digests vbdp/server
```
...
1. **Download recent backup:**
   ```
   aws s3 cp s3://vbdp-backups/vbdp-$(date +%Y%m%d).sql.gz /tmp/
   ```

2. **Restore to test database:**
   ```
   createdb vbdp_test
   gunzip -c /tmp/vbdp-*.sql.gz | psql vbdp_test
   ```

3. **Verify data integrity:**
   ```
   psql vbdp_test -c "SELECT COUNT(*) FROM apps;"
   psql vbdp_test -c "SELECT COUNT(*) FROM versions;"
   ```

4. **Check recent data present:**
   ```
   psql vbdp_test -c "SELECT MAX(created_at) FROM versions;"
...
aws s3 cp s3://vbdp-backups/binaries/test-app-1.0.0.bin /tmp/
...
vbdp-rollback --version 1.1.0 --action fallback-to-full
```
...
# AWS RDS:
aws rds failover-db-cluster --db-cluster-identifier vbdp-cluster
...
# Restore from backup
# 1. Stop application
systemctl stop vbdp-server

# 2. Restore database
dropdb vbdp
createdb vbdp
gunzip -c /backups/vbdp-latest.sql.gz | psql vbdp

# 3. Restart application
systemctl start vbdp-server
```
...
1. **Pause rollout immediately:**
   ```
   vbdp-rollback --version 1.1.0 --action pause --emergency
   ```

2. **Isolate affected version:**
   ```
   # Remove from download servers
   aws s3 rm s3://vbdp-updates-prod/binaries/myapp-1.1.0.bin
   aws s3 rm s3://vbdp-updates-prod/diffs/myapp-*-to-1.1.0.diff
   ```
...
```
[databases]
vbdp = host=localhost port=5432 dbname=vbdp
...
```
postgresql://vbdp_server:password@localhost:6432/vbdp
```
...
Edit `/etc/vbdp-server/config.toml`:
```toml
[server]
...
expr: (rate(vbdp_updates_total{success="false"}[5m]) / rate(vbdp_updates_total[5m])) > 0.05
for: 10m
severity: critical
annotations:
...
expr: (vbdp_bandwidth_saved_bytes_total / vbdp_bandwidth_total_bytes) < 0.50
for: 1h
severity: warning
annotations:
...
4. **Restore database:**
   ...
   dropdb vbdp
   createdb vbdp
   gunzip -c /backups/vbdp-YYYYMMDD-HHMM.sql.gz | psql vbdp
   ```

5. **Restore object storage (if needed):**
   ```
   aws s3 sync s3://vbdp-backups/binaries/ s3://vbdp-updates-prod/binaries/ --dryrun
...
6. **Verify integrity:**
   ...
   psql vbdp < /opt/vbdp/scripts/verify-integrity.sql
   ```
