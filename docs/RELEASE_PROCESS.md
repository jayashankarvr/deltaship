...
This document describes the release process for Deltaship (Versioned Binary Delta Patcher).

## Overview

Deltaship uses automated GitHub Actions workflows to build, test, and release binaries for multiple platforms. Releases are triggered by pushing version tags to the repository.

## Release Types

### Semantic Versioning

Deltaship follows [Semantic Versioning 2.0.0](https://semver.org/):
...
1. **Final Verification**
   ...
   # Dry-run publish
   cargo publish --dry-run -p deltaship-core
...
3. **Monitor Release Workflow**
   ...
   - Go to GitHub Actions: `https://github.com/jayashankarvr/deltaship/actions`
...
4. **Verify Release Artifacts**
   <!-- TODO: Update URL when repository is published -->
   - Go to GitHub Releases: `https://github.com/jayashankarvr/deltaship/releases`
   - Verify all platform binaries are present:
     - `deltaship-client-v0.1.0-x86_64-unknown-linux-gnu.tar.gz`
     - `deltaship-client-v0.1.0-x86_64-unknown-linux-musl.tar.gz`
     - `deltaship-client-v0.1.0-x86_64-apple-darwin.tar.gz`
     - `deltaship-client-v0.1.0-aarch64-apple-darwin.tar.gz`
     - `deltaship-client-v0.1.0-x86_64-pc-windows-msvc.zip`
     - (Same for `deltaship-publisher` and `deltaship-server`)
   - Verify `SHA256SUMS.txt` is present
   - Download and verify checksums:
     ```bash
     # TODO: Update URL when repository is published
     wget https://github.com/jayashankarvr/deltaship/releases/download/v0.1.0/SHA256SUMS.txt
...
1. **Test Release Binaries**
   ...
   # TODO: Update URL when repository is published
   wget https://github.com/jayashankarvr/deltaship/releases/download/v0.1.0/deltaship-client-v0.1.0-x86_64-unknown-linux-gnu.tar.gz
   tar xzf deltaship-client-v0.1.0-x86_64-unknown-linux-gnu.tar.gz
   ./deltaship-client --version
   ...
   **Note**: Currently set to dry-run. To publish for real:
   ...
   b. Update `.github/workflows/release.yml`:
      ...
      - Ensure dependency order is correct:
        ...
        3. `deltaship-diff`
        4. `deltaship-db`
        5. `deltaship-publisher`
        6. `deltaship-server`
        7. `deltaship-client`

   c. Add delays between publishes:
      ```yaml
      - name: Publish deltaship-core
        run: cargo publish -p deltaship-core --token ${{ secrets.CRATES_IO_TOKEN }}

      - name: Wait for crates.io
        run: sleep 30

      - name: Publish deltaship-crypto
        run: cargo publish -p deltaship-crypto --token ${{ secrets.CRATES_IO_TOKEN }}
      ```
...
tar czf deltaship-client-v0.1.0-x86_64-unknown-linux-gnu.tar.gz deltaship-client
...
<!-- TODO: Update URL when repository is published -->
1. Go to: `https://github.com/jayashankarvr/deltaship/releases/new`
...
# Sign binaries
gpg --detach-sign --armor deltaship-client-v0.1.0-x86_64-unknown-linux-gnu.tar.gz
```
