# Deltaship Integration & Adoption Guide

This guide is everything you need to adopt **Deltaship** (Version-aware Binary Differential
...
> **One-line mental model:** Deltaship is an **auto-updater**, not an installer. Your
> normal installer puts the app on disk the first time; Deltaship then keeps that
...
- [5. The `deltaship-updater` sidecar contract](#5-the-deltaship-updater-sidecar-contract)
...
Deltaship has **three roles**. The vendor runs the first two; the third ships inside
your application.

| Component | Binary | Who runs it | Job |
|---|---|---|---|
| **Publisher** | `deltaship-publisher` | Vendor (build/CI machine) | Register a new version, diff it against prior versions, **sign** it with the vendor's Ed25519 key, upload to the server. |
| **Server** | `deltaship-server` | Vendor (self-hosted) | Dumb host: stores binaries/diffs/signatures on disk, answers "is there an update?". File-based storage, no database. |
| **Client / Updater** | `deltaship-updater` (sidecar) or `deltaship-client` (CLI/daemon) | End-user machine, bundled with your app | Check the server, download a diff (or full binary), **verify hash + signature**, atomically replace the installed binary, keep a backup for rollback. |

```
   VENDOR SIDE                         NETWORK                  END-USER MACHINE
   ───────────                         ───────                  ────────────────
 ┌───────────────┐                                          ┌────────────────────┐
 │ deltaship-publisher│  register / diff / sign / publish        │  your application  │
 │  (in CI)      │ ───────────────┐                         │  + deltaship-updater    │
 └───────────────┘                │                         │  (bundled sidecar) │
                                   ▼                         │  + publisher.pub   │
                            ┌─────────────┐    HTTPS         │    (pinned key)    │
                            │ deltaship-server │◀──(Caddy TLS)──▶ │                    │
...
What happens on a single `deltaship-updater` run:
...
7. **Exit code** tells your app what happened: `0` up-to-date, `2` updated
   (restart me), `1` error. See [§5](#5-the-deltaship-updater-sidecar-contract).
...
Deltaship recognizes exactly **five** platform identifiers (a closed set — anything else
...
You build a separate `deltaship-updater` binary per target you ship, and you publish a
separate Deltaship "version" per platform (each platform has its own binary, hash, and
...
# Binaries: target/release/{deltaship-server, deltaship-publisher, deltaship-client, deltaship-updater}
```
...
KEY=$(docker compose run --rm --no-deps deltaship-server --generate-api-key)
echo "$KEY" | docker compose run --rm --no-deps -T deltaship-server hash-key >> data/api_keys.txt
...
> add `--trust-proxy` to the server command (or `DELTASHIP_TRUST_PROXY=1` in the compose
...
Run these on your build/CI machine, in the directory where you want the `.deltaship/`
project workspace to live.

```bash
# Once per project: create .deltaship/ workspace + Ed25519 keypair + local DB.
deltaship-publisher init --passphrase "$SIGNING_PASSPHRASE"
#   -> .deltaship/keys/signing.key (encrypted, 0600) and .deltaship/keys/public.key

# Once: point the publisher at your server.
deltaship-publisher config set server_url https://updates.example.com
...
deltaship-publisher sign --name myapp --version 1.0.0 --passphrase "$SIGNING_PASSPHRASE"
...
Distribute `.deltaship/keys/public.key` to your application build — it gets **bundled and
pinned** by the updater. **Never** distribute `signing.key`.

### 4.4 Bundle and run the updater in your app

Ship the per-platform `deltaship-updater` binary and `public.key` alongside your app.
...
## 5. The `deltaship-updater` sidecar contract
...
| `--server-url <URL>` | ✅ | Base URL of your `deltaship-server` (use `https://` in production). |
| `--public-key <PATH>` | ✅ | The pinned `public.key` you bundled. Verifies every update. |
| `--data-dir <PATH>` | — | State dir (DB, backups, downloads). Default: `~/.local/share/deltaship/<name>/`. |
...
> downgrades. The CLI `deltaship-client update` and the daemon add an *extra* explicit
...
1. **Spawn the sidecar (recommended, any language/platform).** Bundle
   `deltaship-updater`, run it on startup, branch on the exit code. Works identically
   for C/C++, C#, Go, Python, Node/Electron, Java, etc. See
   **[LANGUAGE_EXAMPLES.md](./LANGUAGE_EXAMPLES.md)**.
2. **Run it as a background service/daemon.** Use `deltaship-client --daemon` (it polls
   on `check_interval_secs`) or your OS service manager. Good for always-on agents.
3. **In-process (Rust only).** Call the `deltaship-client` library API directly for
...
```
on app launch:
    status = spawn_and_wait("deltaship-updater", [--name, --install-path, --server-url, --public-key])
    if status == 2:        # updated
...
- Service integration: `deltaship-client service install [--user]` generates a systemd
  unit; or schedule `deltaship-updater` from a `systemd.timer`/cron.
...
- **You cannot overwrite a running `.exe`'s image.** Use a small **launcher** model:
  a tiny `myapp-launcher.exe` runs `deltaship-updater` first, and only then starts
...
- Code signing: keep your existing Authenticode signing in your build; Deltaship's
...
### Command reference (`deltaship-publisher`)

Global flags: `-v/--verbose` (repeatable), `-q/--quiet`.

| Command | Key flags | Purpose |
|---|---|---|
| `init` | `-p/--passphrase` | Create `.deltaship/` workspace, keypair, DB (run once). |
...
### State & distribution
- Workspace: `.deltaship/` in the current directory — `keys/{signing.key,public.key}`,
  `publisher.db` (SQLite), `diffs/<name>/<from>-to-<to>.patch`. Config lives in the
  DB, not a file.
- Install: `cargo install --locked --path crates/deltaship-publisher` (binary
  `deltaship-publisher`).
...
`--api-key "$DELTASHIP_API_KEY"`). See `docs/integration/CI_CD_INTEGRATION.md` and
...
### Run-the-server flags (`deltaship-server`)
...
| `--trust-proxy` / `DELTASHIP_TRUST_PROXY=1` | off | Derive client IP from the right-most `X-Forwarded-For` hop. **Enable this behind Caddy.** |
| `DELTASHIP_CORS_ORIGINS` | unset = allow all (warns) | Comma-separated allowed origins. |
...
- **What's signed:** the publisher signs the canonical payload
  `b"DELTASHIP-sig-v1\x00"` ++ the raw 32-byte BLAKE3 hash of the binary ++ the UTF-8
  version string. Binding the version means a signature for one version **cannot be
  replayed** for another. One implementation of this payload (`deltaship_crypto::signing_payload`)
...
callbacks, custom UI) instead of spawning the sidecar. Add the `deltaship-client` crate
and use:

```rust
use deltaship_client::{load_config, ClientConfig, UpdateChecker, apply_update};
...
// Open the client DB (from deltaship-db) at config.db_path(); register the managed
// binary with its pinned public key (mirrors what `deltaship-client add` does), then:
...
| Rate-limit `429`s behind a proxy hit everyone | Server isn't trusting the proxy. Add `--trust-proxy` / `DELTASHIP_TRUST_PROXY=1`. |
...
5. **The sidecar has no client-side `--allow-downgrade` guard;** it trusts the
   server's newer-only logic. Use `deltaship-client`/daemon if you want the extra
