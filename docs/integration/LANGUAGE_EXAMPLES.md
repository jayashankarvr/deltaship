# VBDP Integration — Language-Specific Examples

Concrete, copy-pasteable examples for embedding the `vbdp-updater` **sidecar** into
...
1. Bundle the per-platform `vbdp-updater` binary and your `public.key` next to your app.
...
|---|---|---|
| `linux-x86_64` / `linux-aarch64` | `vbdp-updater` | `/opt/myapp/bin/myapp` or `~/.local/bin/myapp` |
| `windows-x86_64` | `vbdp-updater.exe` | `%LOCALAPPDATA%\MyApp\myapp.exe` |
| `macos-x86_64` / `macos-aarch64` | `vbdp-updater` | `MyApp.app/Contents/MacOS/myapp` |
...
@echo off
set "APPDIR=%~dp0"
"%APPDIR%vbdp-updater.exe" --name myapp --install-path "%APPDIR%myapp.exe" ^
...
#ifdef _WIN32
#  include <process.h>
#  define UPDATER "vbdp-updater.exe"
#else
#  include <sys/wait.h>
#  include <unistd.h>
#  define UPDATER "./vbdp-updater"
#endif
...
{
    var exe = OperatingSystem.IsWindows() ? "vbdp-updater.exe" : "vbdp-updater";
...
def run_updater() -> int:
    exe = "vbdp-updater.exe" if os.name == "nt" else "./vbdp-updater"
    proc = subprocess.run([
        exe,
        "--name", "myapp",
        "--install-path", "/opt/myapp/bin/myapp",
...
function runUpdater() {
  const exe = process.platform === "win32" ? "vbdp-updater.exe" : "vbdp-updater";
...
public final class Updater {
    public static int runUpdater() throws IOException, InterruptedException {
        ...
        String exe = win ? "vbdp-updater.exe" : "./vbdp-updater";
...
fn run_updater() -> i32 {
    let exe = if cfg!(windows) { "vbdp-updater.exe" } else { "./vbdp-updater" };
    Command::new(exe)
        .args([
            "--name", "myapp",
            "--install-path", "/opt/myapp/bin/myapp",
...
├─ myapp.exe            ← the actual app; vbdp-updater's --install-path
├─ vbdp-updater.exe
└─ publisher.pub
```
...
```text
1. run vbdp-updater.exe --install-path myapp.exe ...   (myapp.exe is NOT running yet)
