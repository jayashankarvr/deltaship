...
This document describes the database migration strategy for Deltaship.

## Overview

Deltaship uses SQLite for both the publisher and client databases. To manage schema changes over time, we use a migration-based approach using sqlx migrations.
...
-- Description: Initial schema for Deltaship Client database
-- Created: 2026-01-14
...
Deltaship currently uses an **inline migration system** where schema is created programmatically in the `init()` functions of `ClientDb` and `PublisherDb`. The migration SQL files serve as:
...
In future versions (v0.2.0+), Deltaship will support:
