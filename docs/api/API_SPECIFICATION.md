...
**Document:** Complete REST API specification for VBDP Update Server
...
The VBDP Update Server provides a RESTful HTTP API for:
...
**Headers:**
- `X-VBDP-Hash`: `blake3:a1b2c3d4e5f6...` (for integrity verification)
- `X-VBDP-Size`: `1200000` (for progress tracking)
...
**Headers:**
- `X-VBDP-Hash`: `blake3:f7g8h9i0j1k2...`
- `X-VBDP-Size`: `85400000`
- `X-VBDP-Version`: `1.1.0`
...
**Headers:**
```
X-VBDP-API-Key: vbdp_publisher_a1b2c3d4e5f6...
X-VBDP-Timestamp: 1704628496
X-VBDP-Signature: hmac_sha256(api_secret, "{timestamp}:{request_body}")
```
...
```json
{
  ...
  "upload_urls": {
    "binary": {
      "url": "https://s3.amazonaws.com/vbdp-updates-prod/upload/...",
      "method": "PUT",
      "headers": {
        "Content-Type": "application/octet-stream"
      },
...
- Deprecated endpoints return warning header:
  ```
  X-VBDP-Deprecation: This endpoint is deprecated. Use /api/v2/check-update instead. Removal date: 2026-07-01
  ```
