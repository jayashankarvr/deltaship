...
**Document:** Operational runbooks for Deltaship incident response
...
This document provides step-by-step procedures for responding to common Deltaship incidents. Each runbook includes diagnosis steps, resolution procedures, and prevention strategies.
...
- **Technical Lead:** maintainers@deltaship.example.com
- **Security Team:** security@deltaship.example.com
- **Infrastructure:** infrastructure@deltaship.example.com
...
- Prometheus shows `up{job="deltaship-server"} == 0`
...
2. **Check recent logs for crash:**
   ```bash
   journalctl -u deltaship-server -n 100 --no-pager
   ```
...
2. **Verify startup:**
   ```bash
   systemctl status deltaship-server
   journalctl -u deltaship-server -f
   ```
...
1. **Check for deadlock (get stack trace):**
   ...
   sudo kill -QUIT $(pgrep deltaship-server)
   journalctl -u deltaship-server -n 200
   ```
...
1. **Check configuration:**
   ```bash
   deltaship-server --config /etc/deltaship-server/config.toml --check-config
   ```

2. **Check file permissions:**
   ```bash
   ls -la /etc/deltaship-server/config.toml
   ls -la /var/lib/deltaship/
   ```

3. **Fix permissions if needed:**
   ```bash
   sudo chown -R deltaship:deltaship /var/lib/deltaship/
   sudo chmod 600 /etc/deltaship-server/config.toml
   ```

4. **Try starting manually for debugging:**
   ```bash
   sudo -u deltaship /usr/local/bin/deltaship-server --config /etc/deltaship-server/config.toml
   ```
...
1. **Enable automatic restart on failure:**
   ```ini
   # In /etc/systemd/system/deltaship-server.service
   [Service]
   Restart=on-failure
   RestartSec=5s
   ```
...
2. **Test connection from Deltaship server:**
   ```bash
   psql -h db-host -U deltaship_server -d deltaship -c "SELECT 1;"
   ```
...
1. **Verify credentials:**
   ```bash
   cat /etc/deltaship-server/config.toml | grep -A 5 "\[database\]"
   ```
...
3. **Reset password if needed:**
   ```sql
   ALTER USER deltaship_server WITH PASSWORD 'new_password';
   ```

4. **Update Deltaship configuration:**
   ```bash
...
1. **Database accepting connections:**
   ```bash
   psql -h db-host -U deltaship_server -d deltaship -c "SELECT version();"
   ```

2. **Deltaship server connected:**
   ```bash
   journalctl -u deltaship-server -n 50 | grep -i database
   ```
...
1. **Check error breakdown by endpoint:**
   ```promql
   sum by (endpoint, status) (rate(deltaship_api_requests_total{status=~"5.."}[5m]))
   ```

2. **Review recent logs:**
   ```bash
   journalctl -u deltaship-server --since "10 minutes ago" | grep -i error
   ```

3. **Check recent deployments:**
   ```bash
   # Check last deployment time
   systemctl show deltaship-server --property=ActiveEnterTimestamp
   ```
...
4. **Check dependencies:**
   ...
   # Storage
   aws s3 ls s3://deltaship-updates-prod/
   ```
...
1. **Rollback to previous version:**
   ...
   sudo systemctl stop deltaship-server

   # Restore previous binary
   sudo cp /usr/local/bin/deltaship-server.backup /usr/local/bin/deltaship-server
...
3. **Check Deltaship server memory usage:**
   ```bash
   ps -o pid,user,%mem,rss,cmd -C deltaship-server
   ```

4. **Look for memory leaks in logs:**
   ```bash
   journalctl -u deltaship-server | grep -i "memory\|allocation\|leak"
   ```
...
1. **Restart Deltaship server to free memory:**
   ```bash
   sudo systemctl restart deltaship-server
   ```
...
1. **Reduce worker count:**
   ```toml
   # In /etc/deltaship-server/config.toml
   [server]
...
1. **Check disk usage:**
   ```bash
   df -h /var/lib/deltaship
   ```

2. **Find largest directories:**
   ```bash
   du -sh /var/lib/deltaship/* | sort -h | tail -10
   ```

3. **Find large files:**
   ```bash
   find /var/lib/deltaship -type f -size +100M -exec ls -lh {} \; | sort -k5 -h
   ```
...
1. **Remove old logs:**
   ```bash
   # Find old logs
   find /var/log/deltaship -name "*.log.*" -mtime +7

   # Delete them
   find /var/log/deltaship -name "*.log.*" -mtime +7 -delete
   ```

2. **Clean temporary files:**
   ```bash
   rm -rf /var/lib/deltaship/tmp/*
   ```

3. **Remove old version diffs (be careful!):**
   ```bash
   # List versions to identify old ones
   ls -lht /var/lib/deltaship/storage/diffs/

   # Remove diffs older than 90 days
   find /var/lib/deltaship/storage/diffs/ -mtime +90 -type f -delete
   ```
...
1. **Archive old binaries to S3:**
   ...
   aws s3 sync /var/lib/deltaship/storage/binaries/ s3://deltaship-archive/ \
     --exclude "*" --include "*/v1.*"

   # Delete local copies after verification
   rm -rf /var/lib/deltaship/storage/binaries/v1.*
   ```
...
2. **Or add additional volume:**
   ```bash
   # Mount new volume
   sudo mount /dev/xvdg /var/lib/deltaship/storage-new
   ...
   # Migrate data
   sudo rsync -av /var/lib/deltaship/storage/ /var/lib/deltaship/storage-new/
...
2. **Write operations work:**
   ```bash
   touch /var/lib/deltaship/test && rm /var/lib/deltaship/test
   ```
...
3. **Regular cleanup automation:**
   ```bash
   # Add to cron: /etc/cron.daily/deltaship-cleanup
   #!/bin/bash
   find /var/log/deltaship -name "*.log.*" -mtime +7 -delete
   find /var/lib/deltaship/tmp -mtime +1 -delete
   ```
...
1. **Identify slow endpoints:**
   ...
   histogram_quantile(0.99,
     sum by (endpoint, le) (rate(deltaship_api_request_duration_seconds_bucket[5m]))
   )
   ```
...
4. **Check network latency:**
   ...
   # To storage
   time aws s3 ls s3://deltaship-updates-prod/ | head -10
   ```
...
1. **Latency improved:**
   ```promql
   histogram_quantile(0.99,
     rate(deltaship_api_request_duration_seconds_bucket[5m])
   ) < 1
   ```
...
1. **Check failure breakdown:**
   ```promql
   sum by (reason) (rate(deltaship_updates_total{success="false"}[5m]))
   ```

2. **Review client error logs:**
   ```bash
   # If you have centralized logging
   grep "update failed" /var/log/deltaship/client-*.log
   ```

3. **Check signature verification:**
   ```promql
   sum(rate(deltaship_signature_verification_failures_total[5m]))
   ```
...
1. **Check diff quality:**
   ...
   - Test locally: `deltaship-test --from X --to Y`
...
1. **Success rate > 99%:**
   ...
   sum(rate(deltaship_updates_total[5m])) > 0.99
   ```
...
3. **Verify signatures manually:**
   ...
   deltaship-verify --binary myapp-1.2.0 --signature sig --public-key public.key
   ```
...
3. **Publisher re-signs and re-publishes:**
   ```bash
   deltaship-sign --version 1.2.0
   deltaship-publish --version 1.2.0 --force
   ```
...
3. **Follow security incident procedure:**
   ...
   - Contact security@deltaship.example.com

### Verification

1. **Signature verification working:**
   ```bash
   deltaship-verify --binary test.bin --signature test.sig --public-key public.key
   ```

2. **Clients accepting updates:**
   ```bash
   # Monitor success rate
   watch -n 5 'curl -s "http://prometheus:9090/api/v1/query?query=deltaship_updates_total"'
   ```
...
2. **Generate reverse diff if needed:**
   ```bash
   # Publisher side
   deltaship-register --version 1.2.0-rollback --binary ./v1.1.0/myapp
   deltaship-sign --version 1.2.0-rollback
   deltaship-publish --version 1.2.0-rollback --rollback-from 1.2.0
   ```
...
1. **Track rollback progress:**
   ```promql
   sum(deltaship_updates_total{app="myapp", to_version="1.1.0"})
   ```
...
1. **Users on safe version:**
   ```promql
   sum by (version) (deltaship_client_version{app="myapp"})
   ```
...
- **Technical Lead:** lead@deltaship.example.com
- **Operations Manager:** ops@deltaship.example.com
...
### Specialized Teams
- **Security:** security@deltaship.example.com
- **Database:** dba@deltaship.example.com
- **Infrastructure:** infra@deltaship.example.com
- **Network:** netops@deltaship.example.com
...
- **Monitoring Dashboard:** https://grafana.example.com/d/deltaship-overview
