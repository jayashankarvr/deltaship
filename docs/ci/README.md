# Publishing to VBDP from GitHub Actions
...
and publishes each binary to a VBDP server when you push a `vX.Y.Z` tag.
...
| Secret | What it is |
| --- | --- |
| `VBDP_SERVER_URL` | Base URL of your `vbdp-server`, e.g. `https://updates.example.com`. |
| `VBDP_API_KEY` | Plaintext API key issued by your server admin. The server stores its Argon2id hash in `<data-dir>/api_keys.txt`; the client sends the plaintext in the `X-API-Key` header. |
| `VBDP_SIGNING_KEY` | Full contents of `.vbdp/keys/signing.key` (the encrypted Ed25519 PEM file produced by `vbdp-publisher init` or `keygen`). Paste the file verbatim including the `-----BEGIN VBDP ENCRYPTED SIGNING KEY-----` headers. |
| `VBDP_SIGNING_PASSPHRASE` | Passphrase that decrypts `VBDP_SIGNING_KEY`. Minimum 12 characters. |

## How signing keys flow through CI

`vbdp-publisher` keeps signing keys at `.vbdp/keys/signing.key`, encrypted
...
1. **Locally, once:** run `vbdp-publisher init --passphrase '<passphrase>'`
   on your workstation (or a one-shot trusted machine). This produces:
   - `.vbdp/keys/signing.key` — the encrypted private key. Paste its full
     contents into the `VBDP_SIGNING_KEY` GitHub secret.
   - `.vbdp/keys/public.key` — the public key. Ship this with your app
     bundle so `vbdp-updater` can verify downloads.
2. **In CI:** the workflow runs `vbdp-publisher init` with the real
   passphrase to bootstrap `.vbdp/publisher.db` (which `register` requires),
...
3. `vbdp-publisher sign --passphrase "$VBDP_SIGNING_PASSPHRASE"` decrypts
   the key in-process and signs the BLAKE3 hash recorded by `register`.

Never commit `.vbdp/keys/signing.key` or the passphrase to your repo. Keep
...
- The **build matrix** — add/remove platforms. If you change the matrix,
  ...
  identifiers (from `vbdp-core`): `linux-x86_64`, `linux-aarch64`,
  `windows-x86_64`, `macos-x86_64`, `macos-aarch64`.
- `VBDP_REF` — pin to a specific commit/tag of the `jayashankarvr/vbdp`
  repo to make CI reproducible. Defaults to `main` because there are no
  binary releases of `vbdp-publisher` yet.

## Known gaps in the publisher CLI

- **No release of `vbdp-publisher`.** The workflow builds it from source on
  ...
  call against `jayashankarvr/vbdp`.
