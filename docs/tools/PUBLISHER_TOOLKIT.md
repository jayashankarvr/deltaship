...
**Configuration:**
- Stored in `.deltaship/config.toml`
- Human-readable format
- Version controlled (except private key)
- Environment variable overrides supported

### Tool 2: `deltaship-register`
...
**Location:** `.deltaship/config.toml`
...
api_key_env = "DELTASHIP_API_KEY"  # Read from environment
upload_timeout = 300  # seconds
retry_attempts = 3

[signing]
algorithm = "Ed25519"
private_key = ".deltaship/keys/private.key"
public_key = ".deltaship/keys/public.key"
key_encrypted = true
...
2. Run `deltaship-register` to add to registry
3. Run `deltaship-sign` to create signatures
4. Run `deltaship-test` to validate
5. Run `deltaship-publish` to deploy
...
2. CI builds binary
3. CI runs `deltaship-register --version $TAG`
4. CI runs `deltaship-sign` (private key from secret store)
5. CI runs `deltaship-test --suite quick`
6. CI runs `deltaship-publish` (if tests pass)
...
**GitHub Actions:**
- Action available: `deltaship-actions/publish@v1`
- Secrets: `DELTASHIP_SIGNING_KEY`, `DELTASHIP_API_KEY`
- Workflow templates provided

**GitLab CI:**
- Docker image: `deltaship/publisher:latest`
- Variables: `DELTASHIP_SIGNING_KEY`, `DELTASHIP_API_KEY`
- Example `.gitlab-ci.yml` provided

**Jenkins:**
- Plugin available: `deltaship-publisher`
...
**Integration:**
- Post-build hook to run `deltaship-register`
- Pre-release hook to run `deltaship-test`
- Release hook to run `deltaship-publish`
