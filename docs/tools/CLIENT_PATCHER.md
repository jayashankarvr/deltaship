...
- Location: `/var/lib/vbdp/versions.db` (Linux)
- Lightweight schema
...
**Configuration File:**
- Location: `/etc/vbdp/config.toml` (system) or `~/.config/vbdp/config.toml` (user)
...
**Service Integration:**
- **systemd service:** `/lib/systemd/system/vbdp.service`
...
**File Locations:**
- Binary: `/usr/bin/vbdp`
- Config: `/etc/vbdp/config.toml`
- Data: `/var/lib/vbdp/`
- Cache: `/var/cache/vbdp/`
- Logs: `/var/log/vbdp/` or journald

**Permissions:**
- Runs as dedicated user (`vbdp`)
...
**File Locations:**
- Binary: `C:\Program Files\VBDP\vbdp.exe`
- Config: `C:\ProgramData\VBDP\config.toml`
- Data: `C:\ProgramData\VBDP\data\`
- Cache: `C:\Users\{User}\AppData\Local\VBDP\cache\`
...
**Service Integration:**
- launchd daemon (`/Library/LaunchDaemons/com.vbdp.patcher.plist`)
- Login item (user-level alternative)
- Menu bar app (optional UI)

**Installation:**
- .pkg installer (native)
- Homebrew formula (`brew install vbdp`)
- DMG with app bundle

**File Locations:**
- Binary: `/usr/local/bin/vbdp`
- Config: `/Library/Application Support/VBDP/config.toml`
- Data: `/var/lib/vbdp/`
- Cache: `~/Library/Caches/VBDP/`
- Logs: `/var/log/vbdp/` or Console.app
...
- `vbdp check` - Check for updates now
- `vbdp status` - Show current status
- `vbdp history` - View update history
- `vbdp config` - View/edit configuration
- `vbdp pause` - Pause updates
- `vbdp resume` - Resume updates
...
- VBDP: Download 1MB + patch = 20 seconds
...
- `vbdp diagnose` - Run health checks
- `vbdp verify` - Verify installed software integrity
- `vbdp reset` - Reset to defaults (clears cache, resets config)

**Logs:**
- Check logs for errors
- Increase verbosity (`vbdp --verbose`)
