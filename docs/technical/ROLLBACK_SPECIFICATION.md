...
This document **unifies** the rollback mechanism specification for VBDP, resolving contradictions found in earlier documentation.
...
**Unified approach:**
VBDP supports **BOTH mechanisms**, used in different scenarios:
...
1. Client creates backup of current version
   /usr/bin/myapp (1.0.0) → /var/lib/vbdp/backups/myapp-1.0.0.backup
...
5. If failed:
   - ROLLBACK: Restore backup
     /var/lib/vbdp/backups/myapp-1.0.0.backup → /usr/bin/myapp
   - Delete myapp.new
   - Log error
```
...
**Location:**
- Linux: `/var/lib/vbdp/backups/`
- Windows: `%PROGRAMDATA%\VBDP\backups\`
- macOS: `/Library/Application Support/VBDP/backups/`
...
```bash
# Register versions
vbdp-register --version 1.0.0 --binary ./myapp-1.0.0
vbdp-register --version 1.1.0 --binary ./myapp-1.1.0

# Publish with bidirectional diffs
vbdp-publish --version 1.1.0 --with-reverse-diffs
```
...
vbdp-rollback --from 1.1.0 --to 1.0.0 --reason "Critical security fix"
```
...
# Backup-based rollback
backup_enabled = true
backup_dir = "/var/lib/vbdp/backups"
backup_retention_count = 3
backup_retention_days = 30
backup_max_total_size_mb = 1024
...
VBDP uses a **hybrid rollback approach**:
