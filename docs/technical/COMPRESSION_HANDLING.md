...
- [Deltaship Solution Strategy](#deltaship-solution-strategy)
...
## Deltaship Solution Strategy

Deltaship uses a **multi-strategy approach**:
...
### Deltaship Recommended Strategy
...
deltaship-register --version 1.0.0 --binary mypackage.tgz

# Publish version 1.0.1 (tiny change)
npm pack  # Creates mypackage-1.0.1.tgz
deltaship-register --version 1.0.1 --binary mypackage.tgz
```

**Deltaship processing:**
...
**Bandwidth savings:**
- **Without Deltaship**: 1.2MB (full download)
- **With Deltaship**: 2KB (diff)
- **Savings**: 99.8%
...
**Bandwidth savings:**
- **Without Deltaship**: 160MB (full image)
- **With Deltaship**: 100KB (diff for changed layer)
- **Savings**: 99.9%

### Integration with Docker Registry

**Future enhancement:** Deltaship server acts as Docker registry proxy

```bash
# Configure Docker to use Deltaship proxy
export DOCKER_REGISTRY=deltaship-proxy.example.com

# Pull image (Deltaship applies diff transparently)
...
Deltaship solves the **compression problem** through:
