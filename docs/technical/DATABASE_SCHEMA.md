...
2. **Current Implementation**: The SQLite schemas for Publisher and Client databases are **currently implemented** in the codebase at:
   - `/crates/vbdp-db/src/schema.rs` - SQLite table definitions
   - `/crates/vbdp-db/src/client.rs` - Client database operations
   - `/crates/vbdp-db/src/publisher.rs` - Publisher database operations (if exists)
...
| **Publisher Toolkit** | SQLite | ✅ Implemented | `crates/vbdp-db/src/schema.rs` |
| **Client Patcher** | SQLite | ✅ Implemented | `crates/vbdp-db/src/schema.rs` |

**For current implementation details**, always refer to:
- **Source of Truth**: `/crates/vbdp-db/src/schema.rs`
- **API Usage**: `/crates/vbdp-db/src/client.rs` and `/crates/vbdp-db/src/publisher.rs`
...
VBDP is designed to use different database systems for different components:
...
The current VBDP implementation uses only SQLite databases (see Publisher and Client sections).

**Database Name:** `vbdp_server`
...
Stores registered publishers (software vendors using VBDP).
...
**✅ CURRENTLY IMPLEMENTED** - See `crates/vbdp-db/src/schema.rs` for actual schema

**Database File:** `.vbdp/publisher.db`
**SQLite Version:** 3.35+
...
**Database File:** `/var/lib/vbdp/client.db` (Linux) or `%PROGRAMDATA%\VBDP\client.db` (Windows)
**SQLite Version:** 3.35+
**Journal Mode:** WAL
**Foreign Keys:** ENABLED
...
```bash
# Apply migrations
vbdp-migrate --database server --up

# Rollback (if supported)
vbdp-migrate --database server --down

# Check status
vbdp-migrate --database server --status
```
...
CREATE USER vbdp_readonly WITH PASSWORD 'secure_password';
GRANT CONNECT ON DATABASE vbdp_server TO vbdp_readonly;
GRANT USAGE ON SCHEMA public TO vbdp_readonly;
GRANT SELECT ON ALL TABLES IN SCHEMA public TO vbdp_readonly;

-- Create application user with limited permissions
CREATE USER vbdp_app WITH PASSWORD 'secure_password';
GRANT CONNECT ON DATABASE vbdp_server TO vbdp_app;
GRANT USAGE ON SCHEMA public TO vbdp_app;
GRANT SELECT, INSERT, UPDATE ON ALL TABLES IN SCHEMA public TO vbdp_app;
GRANT DELETE ON download_logs, update_requests TO vbdp_app;
```
