...
This document provides a **comprehensive storage cost model** for Deltaship deployments, addressing the critical issue identified in the original analysis: **Storage costs grow significantly over time and can exceed initial estimates by 10-100x**.
...
**For Deltaship:** R2 may be more cost-effective due to free egress (downloads)
...
**Without Deltaship (full downloads):**
```
500,000 × 150MB = 75TB
...
```toml
# /etc/deltaship/server.toml

[storage]
provider = "s3"  # or "r2", "gcs", "azure"
bucket = "deltaship-binaries"
region = "us-east-1"
...
- alert: StorageCostHigh
  expr: deltaship_storage_cost_usd > 800
  for: 24h
  labels:
    severity: warning
  annotations:
    summary: "Deltaship storage costs approaching limit"
    description: "Current: ${{ $value }}, Limit: $1000"

- alert: StorageGrowthRapid
  expr: rate(deltaship_storage_total_gb[7d]) > 10
  for: 24h
  labels:
    severity: warning
  annotations:
