...
**Document:** Setup procedures for software publishers using VBDP
...
This guide describes how software publishers (application developers, vendors) set up the VBDP Publisher Toolkit to distribute efficient updates for their applications.
...
**Tools Included:**
- `vbdp-init` - Initialize publisher project
- `vbdp-register` - Register new version
- `vbdp-sign` - Sign diffs and binaries
- `vbdp-test` - Test update packages locally
- `vbdp-publish` - Publish to update server
- `vbdp-analyze` - Analyze bandwidth savings
- `vbdp-rollback` - Rollback problematic releases
- `vbdp-stats` - View statistics
...
```
wget https://releases.vbdp.io/publisher/vbdp-publisher-toolkit-1.0.0-1.x86_64.rpm
sudo dnf install vbdp-publisher-toolkit-1.0.0-1.x86_64.rpm
```

**macOS (Homebrew):**

```
brew tap vbdp/publisher
...
Expected output: `vbdp-init 1.0.0`
...
```
# Linux x86_64
wget https://releases.vbdp.io/publisher/vbdp-publisher-toolkit-1.0.0-linux-x86_64.tar.gz

# macOS (Intel)
wget https://releases.vbdp.io/publisher/vbdp-publisher-toolkit-1.0.0-darwin-x86_64.tar.gz

# macOS (Apple Silicon)
wget https://releases.vbdp.io/publisher/vbdp-publisher-toolkit-1.0.0-darwin-arm64.tar.gz

# Windows
wget https://releases.vbdp.io/publisher/vbdp-publisher-toolkit-1.0.0-windows-x86_64.zip
```
...
```
# Linux/macOS
tar -xzf vbdp-publisher-toolkit-*.tar.gz
cd vbdp-publisher-toolkit-*
sudo ./install.sh

# Windows
unzip vbdp-publisher-toolkit-*.zip
cd vbdp-publisher-toolkit-*
install.bat
```
...
export PATH="$HOME/.vbdp/bin:$PATH"

# Windows (add to system PATH via System Properties)
C:\Users\YourName\.vbdp\bin
```
...
```
git clone https://github.com/vbdp/publisher-toolkit.git
...
Navigate to your application's build directory and initialize VBDP:

```
cd /path/to/your/app
vbdp-init --app-name "MyApp" --update-server "https://updates.example.com"
```
...
**What this creates:**
- `.vbdp/` directory in your project root
...
[signing]
private_key_path = ".vbdp/keys/private.key"
public_key_path = ".vbdp/keys/public.key"
...
```
export VBDP_API_KEY="your-api-key-here"
```
...
```
cp .vbdp/keys/public.key src/assets/vbdp-public.key
```
...
```
cat .vbdp/keys/public.key | xxd -i
```
...
```
vbdp-register --version 1.1.0 --binary ./bin/myapp
```
...
- Stores signatures in `.vbdp/signatures/`

**Output:**
```
Signing version 1.1.0...
Using private key: .vbdp/keys/private.key
...
Signature file: .vbdp/signatures/1.1.0.sig
```
...
```
vbdp-publish --version 1.1.0 --dry-run
```
...
```
vbdp-register --version 1.1.0 --binary ./builds/linux/myapp --platform linux
vbdp-register --version 1.1.0 --binary ./builds/windows/myapp.exe --platform windows
vbdp-register --version 1.1.0 --binary ./builds/macos/myapp --platform macos
```

**Sign all:**

```
vbdp-sign --version 1.1.0 --all-platforms
```

**Publish all:**

```
vbdp-publish --version 1.1.0 --all-platforms
```
...
```
vbdp-publish --version 1.1.0 --changelog CHANGELOG-1.1.0.md
```
...
```
vbdp-publish --version 1.1.0 --rollout-percentage 10
```

Only 10% of users will initially receive update. Increase over time:

```
vbdp-publish --version 1.1.0 --rollout-percentage 25
vbdp-publish --version 1.1.0 --rollout-percentage 50
vbdp-publish --version 1.1.0 --rollout-percentage 100  # Full rollout
```
...
```
vbdp-rollback --version 1.1.0 --action pause
```
...
```
vbdp-rollback --version 1.1.0 --action rollback --rollback-to 1.0.1
```
...
```
vbdp-register --version 1.1.1 --binary ./bin/myapp
vbdp-sign --version 1.1.1
vbdp-publish --version 1.1.1
```
  ...
  publish:
    ...
    steps:
      ...
      - name: Register Version
        run: |
          vbdp-register --version ${{ github.ref_name }} --binary ./bin/myapp
      ...
      - name: Publish to Update Server
        ...
        run: |
          vbdp-publish --version ${{ github.ref_name }}
```

**Set GitHub Secrets:**
- `VBDP_PRIVATE_KEY` - Your signing private key
- `VBDP_API_KEY` - Update server API key
...
publish:
  ...
  before_script:
    ...
    - wget https://releases.vbdp.io/publisher/vbdp-publisher-toolkit_1.0.0_amd64.deb
    - dpkg -i vbdp-publisher-toolkit_1.0.0_amd64.deb
  script:
    - vbdp-register --version $CI_COMMIT_TAG --binary ./bin/myapp
    - echo "$VBDP_PRIVATE_KEY" > .vbdp/keys/private.key
    - vbdp-sign --version $CI_COMMIT_TAG
    - vbdp-publish --version $CI_COMMIT_TAG
  variables:
    VBDP_API_KEY: $VBDP_API_KEY
    VBDP_PRIVATE_KEY: $VBDP_PRIVATE_KEY
```

**Set GitLab CI/CD Variables:**
- `VBDP_PRIVATE_KEY` (masked)
- `VBDP_API_KEY` (masked)
...
pipeline {
    ...
    stages {
        ...
        stage('Publish Release') {
            ...
            steps {
                sh '''
                    vbdp-register --version ${TAG_NAME} --binary ./bin/myapp
                    echo "$VBDP_PRIVATE_KEY" > .vbdp/keys/private.key
...
```
vbdp-analyze --from 1.0.0 --to 1.1.0
```
...
1. Generate new key pair:
   ```
   vbdp-init --generate-keys-only --key-path ./new-keys/
   ```
...
```
vbdp-test --from previous-version --to new-version
```

**Test on staging server first:**

Configure staging server in `.vbdp/config.toml`:
```toml
[server.staging]
...
Publish to staging:
```
vbdp-publish --version 1.1.0 --environment staging
```
