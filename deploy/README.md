# VBDP Update Server - Docker Deployment

Turnkey deployment of `vbdp-server` behind Caddy with automatic TLS.
...
| File | Purpose |
| --- | --- |
| `Dockerfile.server` | Multi-stage build of `vbdp-server` (Debian slim runtime, non-root user). |
| `docker-compose.yml` | `vbdp-server` + `caddy` services, internal-only network. |
| `Caddyfile` | TLS termination + reverse proxy to `vbdp-server:8080`. |
...
2. **Create the data directory and seed API keys.** The server reads
   ...
   the `vbdp-server` binary itself:

   - `vbdp-server --generate-api-key` - prints a fresh 64-char hex key.
   - `vbdp-server hash-key <plaintext>` - prints the Argon2id hash to store.
   ...
   ```sh
   mkdir -p data
   docker compose build vbdp-server
   ...
   PLAINTEXT=$(docker compose run --rm --no-deps vbdp-server --generate-api-key)
   echo "$PLAINTEXT"  # <-- copy to your password manager

   # Append the corresponding Argon2id hash to api_keys.txt.
   echo "$PLAINTEXT" \
     | docker compose run --rm --no-deps -T vbdp-server hash-key \
     >> data/api_keys.txt
...
the `vbdp-server` healthcheck reports healthy, traffic is live.
...
```sh
docker compose stop vbdp-server
tar -czf vbdp-data-$(date +%F).tgz data/
docker compose start vbdp-server
```
...
- The container runs as UID 10001 (`vbdp`). If you bind-mount `./data` from
  a host directory owned by another user, fix ownership with
  `sudo chown -R 10001:10001 data/` or the server will fail to write.
- `VBDP_CORS_ORIGINS` is preset in `docker-compose.yml` to your `DOMAIN`.
...
- The `--generate-api-key` and `hash-key` subcommands are documented in
  `crates/vbdp-server/src/main.rs`. There is no separate admin CLI.
