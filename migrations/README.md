# Deltaship Database Migrations

This directory contains SQL migration files for Deltaship databases.
...
Creates the base schema for the Deltaship Client database including:
...
Creates the base schema for the Deltaship Publisher database including:
...
The Deltaship application automatically creates the schema when initializing databases:
...
Deltaship v0.1.0 uses **inline migrations** where the schema is created programmatically in the `init()` functions. These SQL files serve as:
