# VBDP Monitoring Resources

This directory contains production-ready monitoring resources for VBDP infrastructure.

## Contents

### 1. Grafana Dashboard (`grafana-dashboard.json`)

Pre-built Grafana dashboard for comprehensive VBDP monitoring.
...
3. **Via provisioning (recommended for automation):**
   ...
   cat > /etc/grafana/provisioning/dashboards/vbdp.yml <<EOF
   apiVersion: 1
   providers:
     - name: 'VBDP'
       folder: 'VBDP'
...
4. **Verify rules loaded:**
   ...
   - Check for "vbdp_*_alerts" groups
...
curl -G 'http://prometheus:9090/api/v1/query' \
  --data-urlencode 'query=up{job="vbdp-server"} == 0'
...
Example Alertmanager configuration for routing VBDP alerts:
...
**API Metrics:**
- `vbdp_api_requests_total` - Total API requests by endpoint and status
- `vbdp_api_request_duration_seconds` - Request duration histogram
- `vbdp_api_active_connections` - Current active connections

**Update Metrics:**
- `vbdp_updates_total` - Total updates by app, method, success
- `vbdp_update_duration_seconds` - Update duration histogram
- `vbdp_bandwidth_saved_bytes_total` - Bandwidth saved via diffs

**Diff Metrics:**
- `vbdp_diff_generation_duration_seconds` - Diff generation time
- `vbdp_diff_size_bytes` - Diff size by version transition

**Database Metrics:**
- `vbdp_database_query_duration_seconds` - Query latency
- `vbdp_database_connections` - Connection pool state

**Storage Metrics:**
- `vbdp_storage_used_bytes` - Storage usage by type
- `vbdp_storage_operations_total` - Storage operations by type and status
...
1. **Check Prometheus scraping:**
   ...
   ```
   Verify `vbdp-server` target is UP

2. **Check metrics endpoint:**
   ```bash
   curl http://vbdp-server:8080/metrics
   ```
...
- **Discussions:** https://github.com/jayashankarvr/vbdp/discussions
