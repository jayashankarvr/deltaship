...
Instead, report vulnerabilities via email to: security@deltaship.example.com
...
2. Export the public key: `gpg --armor --export security@deltaship.example.com`
...
Deltaship implements a defense-in-depth security architecture with multiple cryptographic layers.
...
Deltaship uses Blake3 for all content integrity verification:
...
For private or enterprise updates, Deltaship supports optional encryption:
...
Deltaship's security model explicitly defines trust boundaries to help users understand security guarantees:
...
2. **Client Installation Integrity**
   - Initial Deltaship client binary must be authentic
...
- **See:** Code comments in `crates/deltaship-client/src/patcher.rs` for detailed security discussion
